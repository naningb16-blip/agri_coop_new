...
<div class="table-card mb-3">
    <div class="p-3">
        <form method="GET" action="<?= BASE_URL ?>/operational" class="row g-2 align-items-end">
            <input type="hidden" name="tab" value="processing">
            <?php if (isset($mine) && $mine): ?><input type="hidden" name="mine" value="1"><?php endif; ?>
            <div class="col-sm-3">
            ...
            <div class="col-sm-2"><button class="btn btn-sm btn-primary w-100 mt-3">Filter</button></div>
            <div class="col-sm-2">
                <a href="<?= BASE_URL ?>/operational?tab=processing<?= $status ? '&status='.$status : '' ?><?= $type ? '&type='.$type : '' ?>&mine=<?= isset($mine) && $mine ? '0' : '1' ?>" class="btn btn-sm w-100 mt-3 <?= isset($mine) && $mine ? 'btn-info' : 'btn-outline-info' ?>">
...
<div class="table-card">
    ...
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            ...
            <tr>
                <td><a href="<?= BASE_URL ?>/operational/processing-detail?id=<?= $b['id'] ?>" class="fw-semibold text-decoration-none"><?= htmlspecialchars($b['batch_number']) ?></a></td>
                ...
                <td class="text-nowrap">
                    <a href="<?= BASE_URL ?>/operational/processing-detail?id=<?= $b['id'] ?>" class="btn btn-xs btn-outline-primary" title="View"><i class="bi bi-eye"></i></a>
                    <?php if ($_SESSION['user']['role'] === 'admin' && in_array($b['status'], ['pending','in_progress'])): ?>
...
function submitBatch() {
    ...
    fetch(BASE_URL + '/operational/create-processing', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(res => {
