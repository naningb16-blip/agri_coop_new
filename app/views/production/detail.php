    ...
    <div class="col-lg-7">
        ...
        <div class="table-card mb-3">
            ...
            <div class="p-3">
                <div class="row g-2">
                    ...
                    <div class="col-sm-4"><div class="text-muted small">Expected Yield</div><div><?= number_format($record['expected_yield'],2) ?> kg</div></div>
                    <div class="col-sm-4"><div class="text-muted small">Actual Yield</div><div class="<?= $record['actual_yield'] ? 'text-success fw-bold' : 'text-muted' ?>"><?= $record['actual_yield'] ? number_format($record['actual_yield'],2).' kg' : '—' ?></div></div>
                    <div class="col-sm-4">
                        <div class="text-muted small">Yield Variance</div>
                        <div>
                            <?php if ($record['actual_yield'] && $record['expected_yield']): 
                                $variance = $record['actual_yield'] - $record['expected_yield'];
                                $percentVariance = ($variance / $record['expected_yield']) * 100;
                                $varClass = $variance >= 0 ? 'text-success' : 'text-danger';
                            ?>
                            <span class="<?= $varClass ?> fw-bold">
                                <?= $variance >= 0 ? '+' : '' ?><?= number_format($variance, 2) ?> kg 
                                (<?= $variance >= 0 ? '+' : '' ?><?= number_format($percentVariance, 1) ?>%)
                            </span>
                            <?php else: ?>
                ...
                <!-- Status Actions -->
                <?php if (!in_array($record['status'],['completed'])): ?>
                <div class="mt-3 d-flex flex-wrap gap-2">
                    <?php if ($record['status'] === 'harvested'): ?>
                    <!-- Update Expected Yield for Harvested Stage -->
                    <button class="btn btn-sm btn-info text-white" data-bs-toggle="collapse" data-bs-target="#updateExpectedForm">Update Expected Yield</button>
                    <div class="collapse w-100 mt-2" id="updateExpectedForm">
                        <div class="row g-2">
                            <div class="col-md-6"><label class="form-label small">Expected Yield (kg)</label><input type="number" id="updateExpectedYield" class="form-control form-control-sm" step="0.01" value="<?= $record['expected_yield'] ?>" placeholder="Enter expected yield"></div>
                            <div class="col-md-6"><label class="form-label small">Current Actual Yield</label><input type="text" class="form-control form-control-sm" value="<?= number_format($record['actual_yield'], 2) ?> kg" readonly></div>
                            <div class="col-12"><button class="btn btn-sm btn-info text-white" onclick="updateExpectedYield()">Update Expected Yield</button></div>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="collapse w-100 mt-2" id="harvestForm">
                        <div class="row g-2">
                            <div class="col-md-4"><label class="form-label small">Expected Yield (kg)</label><input type="number" id="expectedYield" class="form-control form-control-sm" step="0.01" value="<?= $record['expected_yield'] ?>" placeholder="Enter expected yield"></div>
                            <div class="col-md-4"><label class="form-label small">Actual Yield (kg)</label><input type="number" id="actualYield" class="form-control form-control-sm" step="0.01" placeholder="Enter actual yield"></div>
                            ...
                            <div class="col-md-6"><label class="form-label small">Stock to Warehouse</label>
                                <select id="harvestWh" class="form-select form-select-sm"><option value="">None</option>
                                <?php foreach ($warehouses as $w): ?><option value="<?= $w['id'] ?>"><?= htmlspecialchars($w['name']) ?></option><?php endforeach; ?>
                                </select></div>
                            <div class="col-md-6"><label class="form-label small">Variance</label>
                                <input type="text" id="yieldVariance" class="form-control form-control-sm" readonly placeholder="Will calculate automatically">
                            </div>
        ...
        <div class="table-card mb-3">
            ...
            <div class="collapse p-3 border-bottom" id="addInputForm">
                <form action="<?= BASE_URL ?>/production/add-input" method="POST" data-ajax data-reload="true">
    ...
    <div class="col-lg-5">
        ...
        <div class="table-card">
            ...
            <div class="collapse p-3 border-bottom" id="addSchedForm">
                <form action="<?= BASE_URL ?>/production/add-schedule" method="POST" data-ajax data-reload="true">
                    <input type="hidden" name="production_record_id" value="<?= $record['id'] ?>">
                    <div class="row g-2">
...
// Calculate yield variance in real-time
document.addEventListener('DOMContentLoaded', function() {
    const expectedInput = document.getElementById('expectedYield');
    const actualInput = document.getElementById('actualYield');
    const varianceInput = document.getElementById('yieldVariance');
    
    if (expectedInput && actualInput && varianceInput) {
        function calculateVariance() {
            ...
            if (expected > 0 && actual > 0) {
                ...
                varianceInput.value = `${sign}${variance.toFixed(2)} kg (${sign}${percentVariance.toFixed(1)}%)`;
                varianceInput.className = variance >= 0 
                    ? 'form-control form-control-sm text-success fw-bold' 
                    : 'form-control form-control-sm text-danger fw-bold';
            } else {
                varianceInput.value = '';
                varianceInput.className = 'form-control form-control-sm';
            }
        }
        
        expectedInput.addEventListener('input', calculateVariance);
        actualInput.addEventListener('input', calculateVariance);
    }
});
...
function updateStatus(status) {
    ...
    if (status === 'harvested') {
        ...
        const actualYield = document.getElementById('actualYield')?.value || 0;
        
        if (!actualYield || actualYield <= 0) {
            showToast('Please enter actual yield', 'warning');
            return;
        }
        
        fd.append('expected_yield', expectedYield);
        fd.append('actual_yield',   actualYield);
    ...
    fetch(BASE_URL + '/production/status', { method: 'POST', body: fd })
        .then(r => r.json()).then(res => { showToast(res.message, res.success ? 'success' : 'danger'); if (res.success) location.reload(); });
}

function updateExpectedYield() {
    const expectedYield = document.getElementById('updateExpectedYield')?.value || 0;
    
    if (!expectedYield || expectedYield <= 0) {
        showToast('Please enter expected yield', 'warning');
        return;
    }
    ...
    fetch(BASE_URL + '/production/update-expected-yield', { method: 'POST', body: fd })
...
function completeSchedule(id) {
    ...
    fetch(BASE_URL + '/production/update-schedule', { method: 'POST', body: fd })
        .then(r => r.json()).then(res => { showToast(res.message, res.success ? 'success' : 'danger'); if (res.success) location.reload(); });
}
</script>
